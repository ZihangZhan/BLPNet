...
def extract_mask(pred_arr, gt_arr, mask_arr=None):
    ...
    if mask_arr is not None:
        ...
        idx = np.where(mask_vec == 0)[0]

        # 从预测向量和真实标签向量中删除 mask 中对应位置为 0 的元素
...
def calc_auc(pred_arr, gt_arr, mask_arr=None):
    ...
    # 将标签转换为二进制形式
    threshold = np.max(gt_vec) / 2
    gt_binary = np.where(gt_vec > threshold, 1, 0)

    roc_auc = metrics.roc_auc_score(gt_binary, pred_vec)
...
def calc_jac(pred_arr, gt_arr, kernel_size=(1, 1)):
    FP, FN, TP, TN = numeric_score(pred_arr, gt_arr, kernel_size)
    jaccard = TP / (FP + FN + TP + 1e-12)
